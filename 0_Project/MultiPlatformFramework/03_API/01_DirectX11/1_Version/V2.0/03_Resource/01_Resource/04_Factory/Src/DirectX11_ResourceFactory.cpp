...
DirectX11_ResourceFactory::DirectX11_ResourceFactory(
	...
	IDrawCreate(_windowWidth, _windowHeight),
	m_Device(_device),
	...
	const TextureCreateDesc& _desc)
{
	...
	// サイズ計算
	width = static_cast<uint32_t>(_desc.baseRelativeScaleX * m_ScreenWidth);
	width += _desc.absoluteWidth;
	height = static_cast<uint32_t>(_desc.baseRelativeScaleY * m_ScreenHeight);
	height += _desc.absoluteHeight;
