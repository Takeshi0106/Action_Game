...
#include "WindowsHandle.h"  // ウィンドウハンドル
#include "Windows_Input.h"  // Windows用入力情報取得クラス
#include "Windows_CursorController.h" // Windows用カーソル制御クラス
...
// 静的メンバー変数
std::unique_ptr<Windows_Input> PlatformWindowsSystem::m_Input = nullptr;
std::unique_ptr<Windows_CursorController> PlatformWindowsSystem::m_CursorController = nullptr;
...
bool PlatformWindowsSystem::Init()
{
   ...
   m_AppInstance = GetModuleHandle(nullptr);
    ...
    windClass.hCursor = LoadCursor(m_AppInstance, IDC_ARROW);         // クロスカーソル表示する （デザイン）
...
void PlatformWindowsSystem::Uninit()
{
    ...
    {
        UnregisterClass((LPCWSTR)m_WindowClassName, m_AppInstance);
    }
...
bool PlatformWindowsSystem::GameInit()
{
    ...
    // 描画マネージャー作成
    Windows_Handle handle{m_WinInstance};
    m_DrawManager->Init(m_Width, m_Height, &handle);
	m_DrawManager->SetDrawSetting(FillModeSetting::Solid, CullingSetting::Back_Culling);

    // 入力情報作成
    m_Input = std::make_unique<Windows_Input>();
    m_Input->Init();

	// カーソル制御クラス作成
	m_CursorController = std::make_unique<Windows_CursorController>(&handle);
	m_CursorController->SetCursorMode(CursorMode::CursorMode_Normal);

    // モジュールデータ作成
    GameModules modules = {
        static_cast<IDrawManager*>(m_DrawManager),
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_INPUT:
        // 入力イベント受け取り時に呼び出す。
        Windows_Input::HandleRawInput(lp);
        break;
