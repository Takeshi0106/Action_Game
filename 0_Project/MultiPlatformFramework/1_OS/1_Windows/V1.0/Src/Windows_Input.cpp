...
const uint16_t Windows_Input::kKeyCodeToWindowsKeyCode[KeyCode::KeyCode_Max] =
{
	VK::Up,
	VK::Down,
	VK::Left,
...
// キーボード
bool Windows_Input::m_NowKey[256] = {false};
bool Windows_Input::m_OldKey[256] = {false};

// マウスキー
bool Windows_Input::m_NowMouse[MouseCode::Mouse_Max] = {false};
bool Windows_Input::m_OldMouse[MouseCode::Mouse_Max] = {false};

// マウス位置
Vector2 Windows_Input::m_NowMousePos = {};
Vector2 Windows_Input::m_OldMousePos = {};


// ===============================
// 初期化
// ===============================
bool Windows_Input::Init()
{
	// RawInputのデバイス情報設定
...
// キーのイベント受け取り時の関数
// ===============================
void Windows_Input::HandleRawInput(LPARAM lparam)
{
	// データサイズ
	UINT size = 0;
...
// ===============================
// 押された瞬間
bool Windows_Input::GetKeyTrigger(KeyCode key)
{
	if (m_NowKey[kKeyCodeToWindowsKeyCode[key]] &&
		!m_OldKey[kKeyCodeToWindowsKeyCode[key]])
	{
		return true;
	}
...
// 離れた瞬間
bool Windows_Input::GetKeyRelease(KeyCode key)
{
	if (!m_NowKey[kKeyCodeToWindowsKeyCode[key]] &&
		m_OldKey[kKeyCodeToWindowsKeyCode[key]])
	{
...
// 押されている間
bool Windows_Input::GetKeyPress(KeyCode key)
{
	if (m_NowKey[kKeyCodeToWindowsKeyCode[key]] &&
		m_OldKey[kKeyCodeToWindowsKeyCode[key]])
	{
...
// ===============================
// 押した瞬間
bool Windows_Input::GetMouseTrigger(MouseCode button)
{
...
// 離した瞬間
bool Windows_Input::GetMouseRelease(MouseCode button)
{
	if (!m_NowMouse[button] &&
		m_OldMouse[button])
	{
...
// 押されている間
bool Windows_Input::GetMousePress(MouseCode button)
{
	if (m_NowMouse[button] &&
		m_OldMouse[button])
	{
...
// 現在のカーソル位置
Vector2 Windows_Input::GetMousePos()
{
	return m_NowMousePos;
}

// 位置フレーム前のカーソル位置からどれくらい動いたか
Vector2 Windows_Input::GetMouseMoveAmount()
{
