...
public:
	...
	virtual const Handle CreateSRV(const Handle& _textureHandle, const char* name,
		const Format format, const uint32_t mostDetailedMip = 0, const int32_t mipLevels = -1) = 0;
	virtual const Handle CreateRTV(const Handle& _textureHandle, const char* name, const uint32_t mipSlice) = 0;
